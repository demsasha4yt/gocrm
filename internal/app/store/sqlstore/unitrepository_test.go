...
func TestUnitRepository_Create(t *testing.T) {
	...
	s := sqlstore.New(db)
	u := models.TestUnit(t)
	assert.NoError(t, s.Units().Create(context.Background(), u))
...
func TestUnitRepository_Find(t *testing.T) {
	...
	assert.NoError(t, s.Units().Create(context.Background(), u))
	assert.NotNil(t, u.ID)
	unit, err := s.Units().Find(context.Background(), u.ID)
...
func TestUnitRepository_Delete(t *testing.T) {
	...
	assert.NoError(t, s.Units().Delete(context.Background(), u.ID))
	unit, err := s.Units().Find(context.Background(), 100500)
	assert.Error(t, err)
	assert.Nil(t, unit)
}
...
func TestUnitRepository_Update(t *testing.T) {
	...
	assert.NoError(t, s.Units().Update(context.Background(), u.ID, newU))
	...
	assert.Error(t, s.Units().Update(context.Background(), 100500, newU))
}
